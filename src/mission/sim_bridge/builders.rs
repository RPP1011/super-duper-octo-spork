...
pub fn build_default_sim(player_count: usize, enemy_count: usize, seed: u64) -> SimState {
    ...
    for i in 0..player_count {
        ...
        // Heroes are elite combatants: higher stats than basic enemies,
        // and every hero has a small self/ally heal.  Odd-indexed heroes
        // also get a short CC to disrupt enemy DPS.
        let has_cc = i % 2 == 1;
        units.push(UnitState {
            ...
            control_range: if has_cc { 3.0 } else { 0.0 },
            control_duration_ms: if has_cc { 1500 } else { 0 },
            control_cooldown_ms: if has_cc { 6000 } else { 0 },
            control_cast_time_ms: if has_cc { 300 } else { 0 },
